...
void		txt_draw_floor(t_ray *ray, int thr_id, t_xy xy)
{
	...
	curr_f.dx = (curr_f.dy - 0.0) / (HEIGHT / ray->l_h - 0.0);

	curr_f.x = I((curr_f.dx * ray->floor.dx +
			(1.0 - curr_f.dx) * ray->pos.dx) * TXT_W / 1) % TXT_W;
	curr_f.y = I((curr_f.dx * ray->floor.dy +
			(1.0 - curr_f.dx) * ray->pos.dy) * TXT_H / 1) % TXT_H;
